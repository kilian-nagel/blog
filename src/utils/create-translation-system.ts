...
/**
 * The namespace for i18next resources used by Vitesse.
 * All translations handled by Vitesse are stored in the same namespace and Vitesse always use
 * a new instance of i18next configured for this namespace.
 */
export const I18nextNamespace = 'vitesse' as const

export function createTranslationSystem<T extends i18nSchemaOutput>(
  config: Pick<VitesseConfig, 'defaultLocale' | 'locales'>,
 ...
 */
function stripLangRegion(lang: string): string {
  return lang.replace(/-[a-z]{2}/i, '')
}
...
function localeToLang(
  ...
  locales: VitesseConfig['locales'],
  defaultLocale: VitesseConfig['defaultLocale'],
): string {
...
function buildResources<T extends Record<string, string | undefined>>(
  ...
  for (const dict of dictionaries) {
    for (const key in dict) {
      ...
      if (value)
        dictionary[key as keyof typeof dictionary] = value
    }
  }
  return { [I18nextNamespace]: dictionary as BuiltInStrings & T }
}

export type I18nKeys = UserI18nKeys | keyof VitesseApp.I18n

export type I18nT = TFunction<'vitesse', undefined> & {
  all: () => UserI18nSchema
